...
C                                                                            
C       ------ DSSV POLARIZED PARTON DISTRIBUTIONS ------                    
C                                                                            
C       REFERENCE:                                                           
C           "GLOBAL ANALYSIS OF HELICITY PARTON DENSITIES                    
C            AND THEIR UNCERTAINTIES",                                       
C            Phys.Rev.Lett.101:072001,2008                                  
C                                                                            
C       INPUT:                                                               
...
C           M. Stratmann   marco@ribf.riken.jp                               
C           W. Vogelsang   vogelsan@quark.phy.bnl.gov                        
C                                                                            
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C---------------------------------------------------------------------
      SUBROUTINE DSSVFIT(X,Q2,DUV,DDV,DUBAR,DDBAR,DSTR,DGLU,D1,D2)
C---------------------------------------------------------------------
      IMPLICIT NONE
C...
...
C...
     ...
     3          ARRF(NX+NQ) 
      ...
      DOUBLE PRECISION DUV, DDV, DUBAR, DDBAR, DSTR, DGLU, D1,D2
...
C---------------------------
      IMPLICIT NONE
C...
      INTEGER NPART, NX, NQ, NARG,imode
      PARAMETER (NPART=8, NX=47, NQ=30, NARG=2)
C...
      INTEGER NA(NARG)
...
c      if (imode.eq.0) then
      OPEN(UNIT=71,FILE='/eic/u/cvhulse/epic/test_simple/'//
     +     'DSSV.NLO',STATUS='OLD')
c            OPEN(UNIT=71,FILE='DSSV.NLO',STATUS='OLD')
c            OPEN(UNIT=71,FILE='DSSV_PLUS.NLO',STATUS='OLD')
c       else
c            write(6,*) "wrong fit"
c      endif      
...
C
      DO 10 IQ = 1, NQ
         DO 20 IX = 1, NX-1
            ...
            XG1P(IX,IQ)  = PARTON(7,IQ,IX) / (XB1**3 * XB0)
            XG1N(IX,IQ)  = PARTON(8,IQ,IX) / (XB1**4 * XB0)
  20      CONTINUE
         XUF(NX,IQ)  = 0.D0
         XDF(NX,IQ)  = 0.D0
         XUBF(NX,IQ) = 0.D0
