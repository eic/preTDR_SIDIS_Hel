...
C
      ...
      call numberPDF(N)
      IREP=0
C      WRITE(*,*) 'REPLICA=',IREP
      call InitPDF(IREP)
      CALL DSSVINI
      END
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE FUU(IH,ICH,x,q,z,F1)
...
C*******************************************************************
      ...
      IZ=0
      ZMIN = 1.D0
      CALL VEGAS(SUBDIS,1.D-6,3,15000,3,0,0)
      F1 = ERG1
      RETURN
      END
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
      DOUBLE PRECISION FUNCTION SUBDIS(XX)
C ********************************************************************
...
C ***  CALL OF PARTON DISTRIBUTIONS AND FRAGMENTATION FUNCTIONS :
       CALL PARCOM (IORD,IPOL,ITAR,ICHAR,XP0,ZP0,Q2,QQ00,QG00,GQ00)
       CALL PARCOM (IORD,IPOL,ITAR,ICHAR,XP,ZP0,Q2,QQ0,QG0,GQ0)
       CALL PARCOM (IORD,IPOL,ITAR,ICHAR,XP0,ZP,Q2,Q0Q,Q0G,G0Q)
       CALL PARCOM (IORD,IPOL,ITAR,ICHAR,XP,ZP,Q2,QQ,QG,GQ)
C     **********************************************************************
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC      
      SUBROUTINE PARCOM (IORD,IPOL,ITAR,ICHAR,XP,ZP,Q2,QQ,QG,GQ)
...
C     
C     *** PROTON :
            QQP = ( 4.D0 * (UP*DPU+UB*DPUB) + (DO*DPD+DB*DPDB) +
     1           (ST*DPS+ST*DPSB) )/9.D0
            QGP = ( 4.D0 * (UP+UB) + (DO+DB) + (ST+ST) ) * DPG/9.D0
            GQP = GL*(4.D0 * (DPU+DPUB) + (DPD+DPDB) + (DPS+DPSB))/9.D0
C     *** NEUTRON :
            QQN = ( 4.D0 * (DO*DPU+DB*DPUB) + (UP*DPD+UB*DPDB) +
     1           (ST*DPS+ST*DPSB) )/9.D0
            QGN = ( 4.D0 * (DO+DB) + (UP+UB) + (ST+ST) ) * DPG/9.D0
            GQN = GQP
C     
            IF (ITAR.EQ.0) THEN
               QQ = QQP
               QG = QGP
               GQ = GQP
            ELSE IF (ITAR.EQ.1) THEN
               QQ = QQN
               QG = QGN
               GQ = GQN
            ELSE IF (ITAR.EQ.2) THEN
               IF (IPOL.EQ.0) THEN
                  QQ = 0.5 * (QQP+QQN)
                  QG = 0.5 * (QGP+QGN)
                  GQ = 0.5 * (GQP+GQN)
               ELSE
                  QQ = 0.5 * (QQP+QQN) * (1.D0 - 1.5D0*0.058D0)
                  QG = 0.5 * (QGP+QGN) * (1.D0 - 1.5D0*0.058D0)
                  GQ = 0.5 * (GQP+GQN) * (1.D0 - 1.5D0*0.058D0)
               ENDIF
            ELSE
               IF (IPOL.EQ.0) THEN
                  QQ =  (2.0 * QQP+QQN) / 3.0
                  ...
                  GQ =  (2.0 * GQP+GQN) / 3.0
               ELSE
                  QQ =  (2.0 * (-0.027D0) * QQP+ (0.865D0) * QQN) / 3.0
                  QG =  (2.0 * (-0.027D0) * QGP+ (0.865D0) * QGN) / 3.0
                  GQ =  (2.0 * (-0.027D0) * GQP+ (0.865D0) * GQN) / 3.0
               ENDIF
            ENDIF
            QQ = QQ/XP
            QG = QG/XP
